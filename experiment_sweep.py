...
def build_cfg(
    *,
    ...
    if domain_config is None:
        ...
        mlp_update = {
            "type": domain_cfg["mlp"].get("type", "lda"),
            "model_noise": domain_cfg["mlp"].get("model_noise", 0.0),
            "ridge_alpha": domain_cfg["mlp"].get("ridge_alpha", 10.0),
            "n_folds": domain_cfg["mlp"].get("n_folds", 8),
            "window_s": domain_cfg["mlp"].get("window_s", domain_cfg["neural"].get("window_size_s", 8.0)),
            "embargo_s": domain_cfg["mlp"].get("embargo_s", 4.0),
            "early_stopping": domain_cfg["mlp"].get("early_stopping", True),
        }
        for key in (
            "binary_hidden_layer_sizes",
            "ternary_hidden_layer_sizes",
            "regressor_hidden_layer_sizes",
            "reg_activation",
            "binary_alpha",
            "ternary_alpha",
            "reg_alpha",
            ...
            if key in domain_cfg["mlp"]:
                mlp_update[key] = domain_cfg["mlp"][key]
        cfg["mlp"].update(mlp_update)
        cfg["rl"].update(
            {
    ...
    # Manifest / sweep use "pretrain"; training loops use "baseline"
    if cfg["experiment"]["integration_type"] == "pretrain":
        cfg["experiment"]["integration_type"] = "baseline"
