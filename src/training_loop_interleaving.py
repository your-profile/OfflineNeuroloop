...
def train(env:gymnasium.Env, 
    ...
    online_episodes_per_participant = interleaving_n
    ...
    pbar = trange(episodes_num+total_participant_episodes, unit="ep", bar_format=bar_format, ascii=True)
    ...
    for (offline_episode, ((participant, episode), episode_df)) in enumerate(grouped):
        ...
        for offline_step in range(1, n):
            ...
            if 0 not in flags:
                ...
                neural_signal, clf_probs, class_truth, scoreable = utils_rl.classify_fnirs_at_time(
                    clf,
                    processor,
                    rl_timestamp,
                    participant=participant,
                    window_duration_s=window_duration_s,
                    fnirs_channels=fnirs_channel_names,
                ...
                if scoreable and class_truth is not None:
                    ...
                    classes_truth.append(class_truth)
    ...
    post_training_episode_start = combined_episodes

    # ONLINE POST-TRAINING LOOP
    for online_episode in range(online_episodes_per_participant*total_participant_episodes, episodes_num):
        # set seed
        if domain_key == "F":
            state, _ = env.reset(seed=online_seed) #seed for flappy bird
        else:
...
def train_robot(env:gymnasium.Env, 
    ...
    # rewards, timesteps, success rate, optimality predictions
    all_total_rewards, all_episode_steps, all_episode_success, classes_pred, classes_truth = [],[],[],[],[]
    eval_success, combined_steps, combined_episodes = (0.0, 0, 0)
    ...
    for (participant, episode), episode_df in grouped:
        ...
        # ONLINE MID-TRAINING LOOP
        for online_episode in range(0, interleaving_n):
