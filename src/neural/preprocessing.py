...
class DatasetProcessor:
    ...
    def get_fnirs_window(
        ...
        if self.fnirs_df is None or len(self.fnirs_df) < 2:
            return None
        channels = list(fnirs_channels or self.neural_channels or [])
        if not channels:
            return None
        ...
        t0 = target - pd.Timedelta(seconds=float(window_duration_s))
        seg = self.fnirs_df.loc[(idx >= t0) & (idx <= target), channels]
        if len(seg) < 2:
            return None
        span = (seg.index[-1] - seg.index[0]).total_seconds()
        if span < float(window_duration_s) * float(min_fill):
            return None
    ...
    def get_window_label(
        ...
        """
        if self.label_df is None or len(self.label_df) < 2:
            return None, False
        g = str(granularity).lower().strip()
        if g.startswith("c"):
            col = "continuous_label_shifted"
        elif g.startswith("t") or g.startswith("d"):
            col = "ternary_label_shifted"
        else:
        ...
        t0 = target - pd.Timedelta(seconds=float(window_duration_s))
        labs = self.label_df.loc[(idx >= t0) & (idx <= target), col].dropna().to_numpy(dtype=float)
        if len(labs) < 5:
            return None, False

        if g.startswith("c"):
            return float(labs.mean()), True
        if g.startswith("t") or g.startswith("d"):
            vals, counts = np.unique(np.round(labs).astype(int), return_counts=True)
            maj = vals[np.argmax(counts)]
            if counts.max() / len(labs) < min_majority:
                return int(maj), False
            return int(maj), True
        m = float(labs.mean())
        if ambig_lo < m < ambig_hi:
            return int(m >= 0.5), False
