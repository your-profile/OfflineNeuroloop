...
def train(env:gymnasium.Env, 
    ...
    success, combined_steps, combined_episodes, eval_success = (0.0, 0, 0, 0.0)

    # training progress bar
    bar_format = '{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed} < {remaining}, {rate_fmt} | {postfix}]'
    pbar = trange(episodes_num, unit="ep", bar_format=bar_format, ascii=True)

    # INITIAL ONLINE TRAINING LOOP
    ...
    # OFFLINE DATASET FINETUNING LOOP
    ...
    for online_episode in range(int(online_episode+total_participant_episodes), episodes_num):
        # set seed
        if domain_key == "F":
            state, _ = env.reset(seed=seed) #seed for flappy bird
        else:
    ...
    for online_episode in range(total_participant_episodes, episodes_num):
        ...
        for online_step in range(steps):
            ...
            if transition_priority is not None:
                online_ep["priority"].append(priority)
